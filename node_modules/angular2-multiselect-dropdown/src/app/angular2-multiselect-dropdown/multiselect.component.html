<div class="cuppa-dropdown" (clickOutside)="closeDropdown()">
    ...
    <ul [style.maxHeight] = "settings.maxHeight+'px'">
        <li *ngFor="let item of data | listFilter:filter; let i = index;"  class="pure-checkbox">
            <input type="checkbox" (click)="onItemClick(item,i)" [checked]="isSelected(item)" [disabled]="settings.limitSelection == selectedItems?.length && !isSelected(item)"/>
            <label (click)="onItemClick(item,i)">{{item.itemName}}</label>
<input type="number" (change)="item.value=value.value; onItemClick(item,i)" #value placeholder="assign number  (Required)">
        </li>
    </ul>
    </div>
    </div>
